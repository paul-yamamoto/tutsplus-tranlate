...
一人の人物の全ての役割を列挙するのは大変難しい事です。小さな会社になれば大企業では別々の人物に割り当てられる役割りを一人で担わなければいけません。しかし、「役割り」そのものを定義するのは非常に難しい事です。「役割り」とは何でしょうか？私達はどのように「役割り」を見つけ出すのでしょうか？その方法として、対象者(アクター)とその利用者との関係、対象者にどの（変更）要求を持っているかを調べるのが「役割り」を見つけ出す有効なな方法です。
...
単一責任の原則は私達がコードを書くときにいつも念頭に置かなければいけません。クラスやモジュールにそれを適用することにより、より疎結合で依存性の少ないもを作れます。しかしそれには負の側面も有ります。単一責任の原則にもとづいて開発当初から設計ていると、要求を分析しているうちにより多くのアクターを定義したくなります。これは設計の観点からみると危険な徴候です。必要以上に単一責任の原則を適用しようとすると、最適な設計を超えた冗長なチューニングになってきます。それは分かりやす一まとめにしていたモジュールを分かりにくい、バラバラな単位にしてしまいます。
